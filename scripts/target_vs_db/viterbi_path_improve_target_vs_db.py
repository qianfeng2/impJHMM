...
# input of this script is protein sequences consisting of target and source (ID starts with db) sequence, output txt, ref fasta , radius and three estimated parameters.  
...
reffasta=sys.argv[3]#All target sequences in this file is used for determining emission probs of insert.
width=int(sys.argv[4])
delta_input=float(sys.argv[5])
eps_input=float(sys.argv[6])
rho_input=float(sys.argv[7])
...
#with open("results/jump_we_"+str(width)+".txt", 'r') as infile:
...
#return the viterbi path for all target seqs
start=datetime.now()
