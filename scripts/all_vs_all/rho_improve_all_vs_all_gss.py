...
# # This code is to implement improved model compared with Zilversmit.

# input of this script consists of nine parameters: inputfasta(protein sequences,each one will be the target in turn), output_file, radius, input_delta, input_eps
...
#with open("results/we_"+str(width)+".txt", 'r') as infile:
...
#for target_index in range(len(seq_ID)):
    ...
    #llk=calculate_llk_over_rho_grid(sourceID, [delta_input,eps_input], pairstate_frequency,targetID,rho_input,0.001)#term is 0.001, same with Zilversmit et al (2013)
